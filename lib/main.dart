...
class _CounterScreenState extends State<CounterScreen> {
  ...
  void _initMqttService() {
    _antaresService = AntaresMqttService(
      onDataReceived: updateDataFromMqtt,
  ...
  Future<void> connectToMqtt() async {
    try {
      ...
      logger.severe('MQTT initial connection error: $e');
      await Future.delayed(const Duration(seconds: 10));
      if (mounted) connectToMqtt();
    }
  }
  ...
  void updateDataFromMqtt(Map<String, dynamic> data) {
    ...
    setState(() {
      ...
      try {
        _deviceStatus = DeviceStatus.fromJson(data);
      } catch (e) {
        logger.warning('Error parsing device status: $e');
      }
    });
  }
